...
Compatible alternative for anyone who already uses Claude Code: the harness's `ocr delegate` (see `open-code-review-delegate.md`) — no separate LLM config needed.
...
- For complex fixes requiring manual intervention, clearly describe what needs to be accomplished
...
3. Identify the relevant section based on the context
...
By default the first matching user rule replaces the built-in system rule. Set `merge_system_rule: true` when both must be included.
...
Preview which rule applies to a file before reviewing:
...
- **LLM must be configured first** — `ocr review` fails loudly if no LLM is reachable. Always run `ocr llm test` before the first review.
...
- **Untracked files are reviewed in workspace mode** — bare `ocr review` includes staged, unstaged, *and* untracked changes. Stage selectively if you want narrower scope.
- **Large diffs may hit token limits** — files with very large diffs may be truncated. Default `MAX_TOKENS` is 58888 per request.
- **Plan phase triggers at 50 lines** — diffs > 50 changed lines run an extra risk-analysis phase before main review.
- **Don't pass `--audience human`** — streams progress UI that pollutes output. Always use `--audience agent`.
