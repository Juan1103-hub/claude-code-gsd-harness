      ...
      <div className="absolute inset-x-0 bottom-0 mx-auto flex w-full max-w-md flex-col overflow-hidden rounded-t-3xl border-t border-line bg-paper-raised shadow-2xl">
        ...
        <div className="p-4">
          ...
          <div className="mb-4 flex gap-2">
            {Object.entries(HIGHLIGHT_COLORS).map(([name, hex]) => (
              <button
                ...
                aria-label={`Cor ${name}`}
                title={name}
                className="h-10 w-10 rounded-full border-2 border-line transition-transform hover:scale-110 focus-visible:outline-2 focus-visible:outline-accent"
          ...
          <textarea
            ...
            className="mb-3 h-24 w-full resize-none rounded-xl border border-line bg-paper px-3 py-2 text-sm text-ink placeholder:text-ink-faint focus:border-accent focus:outline-none focus:ring-2 focus:ring-accent/20"
          />
          <button
            type="button"
